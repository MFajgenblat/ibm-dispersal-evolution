...
run_ibm <- function(Species_pool_scenario = species_pool_scenario,
  ...
  for (generation in 1:N_generations) {
    ...
    # Mainland rain
    if (generation < Second_species_generation) {
      current_generation <- rbind(current_generation,
                                  do.call(rbind, lapply(1:N_patches, function(i) cbind(Generation = generation, Patch = i, mainland_individuals[resample(which(mainland_individuals[,1] %in% c(First_species)), rpois(1, Lambda), replace = T),,drop = F]))))
    } else {
      current_generation <- rbind(current_generation,
                                  do.call(rbind, lapply(1:N_patches, function(i) cbind(Generation = generation, Patch = i, mainland_individuals[resample(which(mainland_individuals[,1] %in% c(First_species,Second_species)), rpois(1, Lambda), replace = T),,drop = F]))))
    }
    ...
    if (nrow(current_generation) > 0) {
      ...
      if (sum(reproductive_output) > 0) {
        ...
        upcoming_generation <- cbind(Generation = generation,
                                     Patch = sapply(1:sum(reproductive_output), function(i) resample(c(natal_patches[i], 1:N_patches, NA), 1, replace = F, prob = c(1 - dispersal_probabilities[i], dispersal_probabilities[i]*(1-Chi)*c(rep(1/N_patches, N_patches)), dispersal_probabilities[i]*Chi))),
                                     Species = unlist(sapply(1:nrow(current_generation), function(i) rep(current_generation[i,3], reproductive_output[i]), simplify = F)),
                                     mutated_genotypes)
        
        # Dispersal-related mortality: individuals that dispersed into the void (Patch NA) die
        upcoming_generation <- upcoming_generation[!is.na(upcoming_generation[,"Patch"]),,drop = F]
  ...
  return(data.frame(first_species = First_species,
                    second_species = Second_species,
                    second_species_generation = Second_species_generation,
                    replicate_id = replicate_id,
                    history))
  
}

parameter_grid <- expand.grid(first_species = 2:5,
                              second_species = 1,
                              second_species_generation = c(1,seq(10, 500, by = 10)),
                              replicate_id = 1:10)
...
library(furrr)
library(tidyverse)
plan(multisession, workers = 112)
all_histories <- do.call(rbind,
                         (future_map(1:nrow(parameter_grid),
                                     function(run) run_ibm(First_species = parameter_grid$first_species[run],
                                                           Second_species = parameter_grid$second_species[run],
                                                           Second_species_generation = parameter_grid$second_species_generation[run],
...
all_histories %>%
  filter(Generation > 500,
         Patch == 1,
         replicate_id <= 10) %>%
  group_by(Species, first_species, second_species_generation, replicate_id) %>%
  tally() %>%
  pivot_wider(names_from = Species, values_from = n, values_fill = 0) %>%
  mutate(First_species_success = (`2`+`3`+`4`+`5`)/(`1`+`2`+`4`+`3`+`5`)) %>%
  ggplot() +
  geom_tile(aes(y = replicate_id, x = factor(second_species_generation), fill = First_species_success), color = "white", linewidth = 0.1) +
  scale_x_discrete("Time lag (generations)", expand = c(0,0)) +
  scale_y_reverse("Replicate run", expand = c(0,0), breaks = 1:10) +
  scale_fill_viridis_c("Fraction of the eventual community represented by the first colonizing species", limits = c(0,1), breaks = seq(0, 1, by = 0.2), guide = guide_colorbar(title.position = "top", title.hjust = 0.5, barwidth = unit(12, "cm"), barheight = unit(0.3, "cm"))) +
  facet_wrap(~ paste0("First colonizer: species ", first_species), ncol = 1) +
  coord_equal() +
...
ggsave("Figure_S3.png", width = 16, height = 17.5, units = "cm", dpi = 600)
